"""
title: Universal Project Builder Pipeline v5.4
...
[1] DEEP PHASE CHECK  (command: "check phase X" / "periksa phase X" / dll)
    - Beneran ngecek file di disk, bukan cuma baca tabel status
    - Per task yang ✅ di status_dev: baca file, syntax check, truncation check, run test
    - 100% static — 0 LLM call, 0 token cost
    - Output: confirmed ✅ / suspect ⚠️ / failed ❌ per task
    - Auto-suggest `fix phase X` kalau ada yang bermasalah

[2] FIX PHASE  (command: "fix phase X" / "perbaiki phase X" / dll)
    - Ambil semua task ⚠️/❌ dari hasil deep check
    - Fallback: kalau belum pernah di-check, baca langsung dari tabel status_dev
    - Lempar ke batch executor — sama persis kayak `kerjakan P0-003, P0-004`
    - Guard: kalau task gagal 2x berturut-turut → stop, jangan infinite loop

[3] PROMPT DETECTION TIDAK DI-LOCK
    - Semua command detection pakai flexible regex + keyword matching
    - Bukan exact string match — typo, variasi bahasa, dll tetap works
    - "periksa phase", "check phase", "cek mendalam", dll semua ke-detect
    - "perbaiki phase", "fix phase", "ulangi phase", dll semua ke-detect

COMMAND SUMMARY v5.4:
─────────────────────────────────────────────────────────────────
  cek phase P0          → baca tabel status (0 token)
  check phase P0        → deep check file di disk (0 token)
...
class Pipe:
    ...
    async def _write(self, path: str, content: str) -> bool:
        ...
        if not norm.startswith(sandbox):
            logger.warning(f"[v5.3] Write blocked (outside sandbox): {path}")
            return False
        rel = os.path.relpath(norm, sandbox)
        # v5.3: use dynamically discovered dirs; root-level files always allowed
        if self._safe_write_dirs and not any(rel.startswith(d) for d in self._safe_write_dirs):
            # Allow root-level files (no subdir) only if they're .py/.yaml/.yml/.json/.md
            if os.sep in rel or "/" in rel:
                logger.warning(f"[v5.3] Write blocked (dir not discovered): {rel}")
                return False
        try:
            ...
            logger.error(f"[v5.3] Write error {path}: {e}")
            return False
...
"""
    ...
    def _parse_task_ids(self, user_msg: str) -> List[str]:
        ...
        Supported task ID pattern: <PREFIX>-<NUMBER>
          where PREFIX = one or more alphanum/dash segments (e.g. P0, P10, SPRINT-3, PHASE-A)
          and   NUMBER = one or more digits (e.g. 001, 1, 025)

        Examples:
          "kerjakan P0-005"                    → ["P0-005"]
          "kerjakan p1-001"                    → ["P1-001"]
          "kerjakan SPRINT-3-007"              → ["SPRINT-3-007"]
          "kerjakan P10-001 sampai P10-005"    → ["P10-001".."P10-005"]
          "kerjakan P0-004, P0-005, P0-006"    → ["P0-004","P0-005","P0-006"]

        Range expansion: works when both IDs share identical prefix.
        PREFIX is defined as everything before the final -<digits> segment.
        """
        # Regex: capture full ID including multi-segment prefixes like SPRINT-3-007
        # Pattern: word-chars (with internal dashes) ending in -digits
        ...
        def split_prefix_num(tid: str) -> Tuple[str, int]:
            """Split 'P0-005' → ('P0', 5), 'SPRINT-3-007' → ('SPRINT-3', 7)."""
            m = re.match(r"^(.*)-(\d+)$", tid)
            if m:
                return m.group(1), int(m.group(2))
            return tid, 0

        # Detect range: "<ID> sampai/to/hingga/until <ID>"
        ...
        if range_match:
            ...
            if prefix_a == prefix_b and num_a <= num_b:
                ...
                return [f"{prefix_a}-{str(n).zfill(width)}" for n in range(num_a, num_b + 1)]

        # No range — deduplicated list preserving order
        seen: set = set()
        result: List[str] = []
        for tid in all_ids:
            if tid not in seen:
    ...
    # Keywords that signal each command type.
    # Any combination of action + subject = match.
    _CMD_QUICK_CHECK = {
    ...
    def _detect_command(self, user_msg: str) -> Optional[Tuple[str, str]]:
        ...
        Detects action intent + phase prefix from free-form message.

        Returns: ("quick_check" | "deep_check" | "fix_phase", phase_prefix)
                 or None if no phase command detected.

        Detection logic:
          1. Check if message contains any action keyword
          2. Check if message contains any subject keyword (phase/fase/tahap)
             OR just action + phase ID directly (shorthand)
          3. Extract phase prefix (anything that looks like a phase, not a task ID)

        Examples (all detected correctly):
          "cek phase P0"              → (quick_check, P0)
          "status P1"                 → (quick_check, P1)
          "check phase SPRINT-3"      → (deep_check, SPRINT-3)
          "periksa fase P10"          → (deep_check, P10)
          "deep check P0"             → (deep_check, P0)
          "fix phase P0"              → (fix_phase, P0)
          "perbaiki fase SPRINT-3"    → (fix_phase, SPRINT-3)
          "ulangi semua yang gagal P2"→ (fix_phase, P2)
        """
        msg_lo = user_msg.lower()

        # Extract phase prefix candidate from message
        # Phase prefix = alphanum+dash combo that is NOT a full task ID (no trailing -digits)
        ...
        for m in re.finditer(r"[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*", user_msg):
            ...
            if candidate.lower() in {
                ...
                continue
            # Skip if it looks like a full task ID (ends in -digits only)
            if re.match(r"^[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*-\d+$", candidate):
                continue
            # Must start with a letter and have reasonable length
            if re.match(r"^[A-Z]", candidate) and len(candidate) >= 1:
        ...
        # Determine command type by action keywords
        ...
        # Also check if subject keyword present (phase/fase/tahap)
        has_subject = has_any(self._CMD_QUICK_CHECK["subject"])  # same list for all

        # Fix takes priority
        if is_fix and (has_subject or phase_prefix):
            return ("fix_phase", phase_prefix)
        # Deep check
        if is_deep and (has_subject or phase_prefix):
            return ("deep_check", phase_prefix)
        # Quick check (must have subject keyword OR explicit "cek"/"status")
        if is_quick and (has_subject or phase_prefix):
    ...
    def _phase_quick_check(self, phase_prefix: str, all_tasks: List[Dict[str, str]]) -> str:
        ...
        if failed or in_progress:
            ids = ", ".join(t["id"] for t in failed + in_progress)
    ...
    async def _phase_deep_check(
        ...
        v5.4: For each ✅ task in phase, actually verify the written files:
          ...
          3. Truncation markers check (_validate_code)
          4. Run test file if present

        Returns: (report_markdown, list_of_suspect_or_failed_task_ids)
        0 LLM calls.
        """
        def get_prefix(tid: str) -> str:
            m = re.match(r"^(.*)-\d+$", tid)
        ...
        for t in phase_tasks:
            ...
            for rel_path in file_paths:
                if not rel_path or not rel_path.endswith((".py", ".yaml", ".yml", ".json")):
                    continue
                ...
                # Only do deeper checks on Python files
                ...
                for issue in validation:
                    if "not in AVAILABLE_PACKAGES" in issue:
                        task_warnings.append(issue)  # import warnings = soft
                    else:
                        task_issues.append(issue)    # truncation = hard

            # Check 4: run test file
            test_result_str = ""
            # Infer test file path from files list
            test_files = [
                f.strip() for f in re.split(r"[,\n]", files_str)
                if "test" in f.lower() and f.strip().endswith(".py")
            ]
            if test_files:
        ...
        if test_file and not self.valves.READ_ONLY_MODE:
            ...
            if os.path.isfile(test_path):
                ...
                if proceed:
                    ...
                    test_result = output
                    # v5.2: Detect missing package errors — warn instead of hard fail
                    # These are env setup issues, not code bugs
...
"""
    ...
    async def pipe(
        ...
        # v5.3: Refresh dynamic dirs at the start of every invocation
        self._refresh_dirs()

        await self._status(emit, "🚀 Pipeline v5.4 starting...")
        ...
        if cmd:
            ...
            # Load knowledge (needed for all phase commands)
            ...
            # ── Quick check (cek/status/lihat phase X) ───────────
            ...
            # ── Deep check (check/periksa/verifikasi phase X) ────
            elif cmd_type == "deep_check":
                await self._status(emit, f"🔬 Deep check phase {phase_prefix}...")
                report, problem_ids = await self._phase_deep_check(phase_prefix, all_tasks, emit)
                await self._status(emit, "✅ Deep check done", done=True)
                await self._replace(emit, report)
                return report

            # ── Fix phase (fix/perbaiki/ulangi phase X) ──────────
                ...
                # Inject task IDs into user_msg and fall through to batch executor
        ...
        # ── Load knowledge (skip if already loaded by fix_phase above) ──
        if "knowledge" not in dir() or knowledge is None:  # type: ignore[name-defined]
            await self._status(emit, "📖 Loading knowledge files...")
            knowledge = await self._load_knowledge(emit)
            if knowledge is None:
                return "❌ Failed to load knowledge files. Check KNOWLEDGE_PATH valve."
        ...
        batch_header = f"""## Pipeline v5.4 — Batch Complete
