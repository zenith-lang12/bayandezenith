"""
title: Universal Project Builder Pipeline v5.6
author: Claude (Anthropic) — redesigned for BayanDeZenith & beyond (Upgraded by Gemini)
version: 5.6.0
required_open_webui_version: 0.9.6

WHAT'S NEW IN v5.6 — MATURITY & BUG FIXES:
─────────────────────────────────────────────────────────────────
[1] RIGID REGEX FIX: _load_knowledge kini menggunakan re.search dan .endswith(".md"). Tidak akan gagal karena spasi atau variasi nama file.
[2] DECIMAL ID SUPPORT: Sistem sepenuhnya mendukung ID task berdesimal seperti TASK-0.1 atau P0-1.5.
[3] PATCH SABOTAGE FIX: Smart Patch kini bisa emit *hanya* patch tanpa perlu menyertakan array files kosong. Hemat token maksimal.
[4] MEMORY LEAK FIX: Zombie process docker/native pip dibunuh seketika menggunakan proc.kill() saat timeout terjadi.
"""
...
class Pipe:
   class Valves(BaseModel):
       ...
       EXTRA_WRITE_DIRS: str = Field(
           default="",
           description=(
               "Comma-separated extra folders to allow writing to and scanning, "
       ...
       ENABLE_SMART_PATCH: bool = Field(
           default=True,
           description=(
               "Allow Coder to emit search/replace patches instead of full rewrites "
       ...
       ENABLE_AUTO_PIP: bool = Field(
           default=True,
           description=(
               "Auto pip-install missing packages when test fails with "
       ...
       ENABLE_LINTER: bool = Field(
           default=True,
           description=(
               "Run static linter (ruff → flake8 → pyflakes fallback) "
       ...
       ENABLE_DEP_GRAPH: bool = Field(
           default=True,
           description=(
               "Build AST import dependency graph. "
       ...
       ENABLE_ENV_CHECK: bool = Field(
           default=True,
           description=(
               "Check Python env health before tests "
       ...
       PATCH_MIN_LINES: int = Field(
           default=60,
           description=(
               "Minimum file line count before patch mode is preferred over full rewrite."
           ),
       )
   ...
   async def _write(self, path: str, content: str) -> bool:
       ...
       if not norm.startswith(sandbox):
           logger.warning(f"[v5.6] Write blocked (outside sandbox): {path}")
           return False
       ...
       if rel_root in _DIR_BLACKLIST:
           logger.warning(f"[v5.6] Write blocked (blacklisted dir): {rel}")
           return False
       ...
       try:
           ...
           logger.error(f"[v5.6] Write error {path}: {e}")
           return False
   ...
   async def _llm(
       ...
       try:
           ...
           logger.error(f"[v5.6] LLM call error ({model_id}): {e}")
   ...
   def _extract_active_phase(self, content: str, task_id: str, phase_prefix: str) -> str:
       ...
       val_phase_num = ""
       m_num = re.search(r'\d+(?:\.\d+)?', phase_prefix)
       if m_num:
   ...
   async def _scan_files(
       ...
       dep_expand_paths: Optional[List[str]] = None,
       tokens_before: Optional[Dict[str, int]] = None,
   ) -> Dict[str, str]:
   ...
   def _parse_tasks(self, content: str) -> List[Dict[str, str]]:
       ...
       for line in content.split("\n"):
           ...
           if re.match(r"^\|[\s\-:]+\|", s):
               in_table = True
               continue
           if in_table and re.match(r"^\|?\s*[A-Za-z0-9][A-Za-z0-9\-]*-\d+(?:\.\d+)?", s):
   ...
   async def _log_task(
       ...
       try:
           ...
           logger.warning(f"[v5.6] Log write failed: {e}")

   # ──────────────────────────────────────────────────────────
   # v5.6 [T1] SMART DIFF MODIFIER — apply_patch
   # ──────────────────────────────────────────────────────────
   ...
   # ──────────────────────────────────────────────────────────
   # v5.6 [T2] AUTO-DEPENDENCY HEALER — auto_pip_install
   # ──────────────────────────────────────────────────────────
   async def _auto_pip_install(self, package_name: str) -> Tuple[bool, str]:
       ...
       proc_native = None
       try:
           proc_native = await asyncio.create_subprocess_exec(
               sys.executable, "-m", "pip", "install", "--quiet", package_name,
               stdout=asyncio.subprocess.PIPE,
               stderr=asyncio.subprocess.PIPE,
           )
           stdout, stderr = await asyncio.wait_for(proc_native.communicate(), timeout=120)
           if proc_native.returncode == 0:
   ...
   # ──────────────────────────────────────────────────────────
   # v5.6 [T3] ZERO-TOKEN STATIC LINTER — run_static_linter
   # ──────────────────────────────────────────────────────────
   ...
   # ──────────────────────────────────────────────────────────
   # v5.6 [T4] AST IMPORT DEPENDENCY MAPPER
   # ──────────────────────────────────────────────────────────
   def _get_project_dependency_graph(self) -> Dict[str, List[str]]:
       ...
       for rel in all_py:
           ...
           mod_map[dotted] = rel
           mod_map[parts[-1].removesuffix(".py")] = rel
   ...
   # ──────────────────────────────────────────────────────────
   # v5.6 [T5] ENVIRONMENT HEALTH CHECKER
   # ──────────────────────────────────────────────────────────
   ...
   # ──────────────────────────────────────────────────────────
   # v5.6 [T8] TOKEN BUDGET OPTIMIZER
   # ──────────────────────────────────────────────────────────
   ...
   def _dynamic_file_char_limit(self, tokens_before: Optional[Dict[str, int]] = None) -> int:
       ...
       if ratio > 0.6:
           return MAX_FILE_CHARS_FOR_CODER
       elif ratio > 0.35:
           return MAX_FILE_CHARS_FOR_CODER // 2
       else:
           return MAX_FILE_CHARS_FOR_CODER // 4
...
"""
   ...
   def _prompt_code(
       ...
       compressed_knowledge: str,
       relevant_files: Dict[str, str],
       patch_candidates: Optional[List[str]] = None,
       force_unittest: bool = False,
   ) -> str:
       files_section = ""
...
"""
   ...
   async def _load_knowledge(self, emit: Callable) -> Optional[Dict[str, str]]:
       ...
       patterns = [
           r"permanen", r"permanent", r"context",
           r"status", r"progress",
           r"aturan", r"rules", r"sistem",
       ]
       ...
       for pattern in patterns:
           for fname in found_files:
               fname_lo = fname.lower()
               if fname_lo.endswith(".md") and re.search(pattern, fname_lo) and fname not in knowledge:
   ...
   def _parse_task_ids(self, user_msg: str) -> List[str]:
       raw_ids = re.findall(r"[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*-\d+(?:\.\d+)?", user_msg)
       all_ids = [tid.upper() for tid in raw_ids]
       if not all_ids:
           return []

       def split_prefix_num(tid: str) -> Tuple[str, str]:
       ...
       range_match = re.search(
           r"([A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*-\d+(?:\.\d+)?)"
               ...
               try:
                   ...
                   if val_a <= val_b:
                       if num_a.isdigit() and num_b.isdigit():
                           ...
                           expanded = [f"{prefix_a}-{str(n).zfill(width)}" for n in range(int(num_a), int(num_b) + 1)]
                           range_ids = {id_a, id_b}
   ...
   def _detect_command(self, user_msg: str) -> Optional[Tuple[str, str]]:
       ...
       if subject_match:
           ...
           if not re.match(r"^[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*-\d+(?:\.\d+)?$", candidate):
       ...
       if not phase_prefix:
           for m in re.finditer(r"[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*", user_msg):
               ...
               if candidate.lower() in {
                   ...
                   continue
               if re.match(r"^[A-Za-z][A-Za-z0-9]*(?:-[A-Za-z0-9]+)*-\d+(?:\.\d+)?$", candidate):
                   continue
       ...
       for attempt in range(self.valves.MAX_RETRY + 1):
           ...
           if not coder_response or coder_response.startswith("[ERROR]"):
               ...
               if attempt < self.valves.MAX_RETRY:
                   ...
                   self._record_failure(task_id, attempt, reviewer_feedback)
                   continue
               await self._commit_status(
                   knowledge, status_dev_content, task_id,
           ...
           if linter_issues:
               ...
               if attempt < self.valves.MAX_RETRY:
                   ...
                   self._record_failure(task_id, attempt, f"linter: {len(linter_issues)} issues")
                   continue
...
"""
   ...
   async def pipe(
       ...
       self._tokens = {}
       self._env_health = None
       self._task_failure_log = {}

       self._refresh_dirs()

       await self._status(emit, "🚀 Pipeline v5.6 starting...")
       ...
       batch_header = f"""## Pipeline v5.6 — Batch Complete
