...
Bugfixes v4.6.2:
- FIX: .list. object has no attribute .lower. — reviewer_feedback dinormalisasi
  ke string sebelum dipakai (model kadang ngembaliin list dari field harus_diubah)
- FIX: Reviewer prompt dipertegas agar harus_diubah selalu string, bukan array
...
"""
    ...
    def _build_reviewer_prompt(
        ...
        # BUGFIX v4.6.3: Sebelumnya konten file dikirim 2x (di coder_output JSON
        # + di files_section) → reviewer_input meledak ~38k token → model kewalahan
        # → REJECTED tanpa feedback → retry stuck.
        # Fix: hapus raw coder_output (sudah ada di files_section),
        # truncate tiap file supaya reviewer tidak OOM.

        MAX_FILE_PREVIEW = 3000  # chars per file untuk reviewer
        files_section = ""
        for path, content in written_files.items():
            preview = content[:MAX_FILE_PREVIEW]
            truncated = "...[dipotong]" if len(content) > MAX_FILE_PREVIEW else ""
            files_section += f"\n\n### {path}\n```python\n{preview}{truncated}\n```\n"

        if not files_section:
            files_section = "\n(Tidak ada file yang ditulis — READ_ONLY_MODE atau semua write gagal)"

        # Summary dari coder output saja (bukan full JSON yang besar)
        try:
            co = json.loads(re.search(r"\{[\s\S]*\}", coder_output).group(0)) if coder_output else {}
            coder_summary = co.get("summary", "(tidak ada summary)")
            files_written = [f.get("path", "?") for f in co.get("files", [])]
        except Exception:
            coder_summary = "(gagal parse coder output)"
...
**ID**: {task.get('id', 'UNKNOWN')}
**Nama**: {task.get('name', '')}
**Deskripsi**: {task.get('description', task.get('name', ''))[:500]}

# RINGKASAN CODER
- Summary: {coder_summary}
- Files ditulis: {', '.join(files_written) or '(tidak ada)'}

# ISI FILE YANG DITULIS
{files_section}

# TUGAS REVIEWER
1. Review kualitas kode: correctness, best practices, error handling, type hints
2. Cek apakah task selesai sesuai requirement
3. Kalau REJECTED, WAJIB isi harus_diubah dengan feedback konkret dan spesifik

# OUTPUT FORMAT
Return HANYA JSON (tanpa markdown, tanpa penjelasan):
{{
  "verdict": "APPROVED" | "APPROVED_WITH_NOTES" | "REJECTED",
  "catatan": "Catatan singkat sebagai string teks biasa",
  "harus_diubah": "String spesifik hal yang harus diperbaiki, dipisah koma (BUKAN array/list JSON, WAJIB diisi kalau REJECTED)"
}}
"""
        ...
        for attempt in range(max_retry + 1):
            ...
            if "APPROVED" in v:
                ...
                raw_feedback = (
                    ...
                    or verdict.get("catatan", "")
                )
                # BUGFIX v4.6.1: model kadang kembalikan list bukan string
                if isinstance(raw_feedback, list):
                    reviewer_feedback = ", ".join(str(x) for x in raw_feedback)
                elif not isinstance(raw_feedback, str):
                    reviewer_feedback = str(raw_feedback)
                else:
                    reviewer_feedback = raw_feedback

                # BUGFIX v4.6.3: kalau reviewer REJECTED tapi feedback kosong
                # (model kewalahan / context limit), paksa feedback generic
                # supaya coder tahu harus improve apa
                if not reviewer_feedback.strip():
                    reviewer_feedback = (
                        "Reviewer tidak memberikan feedback spesifik. "
                        "Perbaiki: tambah error handling, type hints, docstrings, "
                        "validasi input, dan pastikan semua edge case ditangani."
                    )
                    await self._emit_status(
                        __event_emitter__,
                        "⚠️ Reviewer REJECTED tanpa feedback — pakai fallback feedback",
                    )
